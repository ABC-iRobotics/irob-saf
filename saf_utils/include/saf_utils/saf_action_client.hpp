...
public:
    SafActionClient(rclcpp::Node::SharedPtr node, const std::string& action_name);
...
template <class ActionSpec>
SafActionClient<ActionSpec>::SafActionClient(rclcpp::Node::SharedPtr node, const std::string& action_name)
    : node(node) {
    action_client = rclcpp_action::create_client<ActionSpec>(node, action_name);
}

template <class ActionSpec>
void SafActionClient<ActionSpec>::sendGoal(const Goal& goal) {
    ...
    send_goal_options.goal_response_callback =
        std::bind(&SafActionClient<ActionSpec>::goalResponseCallback, this, std::placeholders::_1);
    send_goal_options.feedback_callback =
        std::bind(&SafActionClient<ActionSpec>::feedbackCallback, this, std::placeholders::_1, std::placeholders::_2);
    send_goal_options.result_callback =
        std::bind(&SafActionClient<ActionSpec>::resultCallback, this, std::placeholders::_1);

    action_client->async_send_goal(goal_msg, send_goal_options);
}

template <class ActionSpec>
void SafActionClient<ActionSpec>::goalResponseCallback(const typename GoalHandle::SharedPtr & goal_handle) {
    if (!goal_handle) {
...
template <class ActionSpec>
void SafActionClient<ActionSpec>::feedbackCallback(typename GoalHandle::SharedPtr, const std::shared_ptr<const Feedback> feedback_msg) {
    feedback = std::make_shared<Feedback>(*feedback_msg);
}

template <class ActionSpec>
void SafActionClient<ActionSpec>::resultCallback(const typename GoalHandle::WrappedResult & result_msg) {
...
template <class ActionSpec>
bool SafActionClient<ActionSpec>::isDone() {
    return done;
}

template <class ActionSpec>
bool SafActionClient<ActionSpec>::isActive() {
    return active;
}

template <class ActionSpec>
typename SafActionClient<ActionSpec>::Feedback SafActionClient<ActionSpec>::getFeedback() {
    return *feedback;
}

template <class ActionSpec>
typename SafActionClient<ActionSpec>::Result SafActionClient<ActionSpec>::getResult() {
    return *result;
}
