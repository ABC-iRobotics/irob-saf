...
protected:
  ...
  // Action server
  std::shared_ptr<rclcpp_action::Server<irob_msgs::action::RobotAction>> action_server;
  ...
  // Publishers
  rclcpp::Publisher<irob_msgs::msg::ToolPoseStamped>::SharedPtr measured_cp_pub;
  ...
  void advertiseHighLevelTopics() {
    // robot interface
    measured_cp_pub = node->create_publisher<irob_msgs::msg::ToolPoseStamped>(
    ...
    instrument_info_pub = node->create_publisher<irob_msgs::msg::InstrumentInfo>(
        "robot/" + arm_name + "/instrument_info", 10);
  }

  void startActionServer() {
    // Action server creation and spin
    auto handle_goal = [this](const std::shared_ptr<rclcpp_action::ServerGoalHandle<irob_msgs::action::RobotAction>> goal_handle) {
      this->robotActionCB(goal_handle);
    };

    action_server = rclcpp_action::create_server<irob_msgs::action::RobotAction>(
...
public:
  ...
  void loadInstrumentInfo(rclcpp::Node::SharedPtr priv_nh) {
    ...
    if (basic_type == "GRIPPER") {
      instrument_info.basic_type = irob_msgs::msg::InstrumentInfo::GRIPPER;
    } else if (basic_type == "SCISSORS") {
      instrument_info.basic_type = irob_msgs::msg::InstrumentInfo::SCISSORS;
    } else if (basic_type == "CAMERA") {
      instrument_info.basic_type = irob_msgs::msg::InstrumentInfo::CAMERA;
    } else {
    ...
    while (priv_nh->get_parameter("instrument/jaw_parts/p" + std::to_string(i) + "/start", probe)) {
      irob_msgs::msg::InstrumentJawPart jaw_part;
      ...
      if (type == "JOINT") {
        jaw_part.type = irob_msgs::msg::InstrumentJawPart::JOINT;
      } else if (type == "GRIPPER") {
        jaw_part.type = irob_msgs::msg::InstrumentJawPart::GRIPPER;
      } else if (type == "SCISSORS") {
        jaw_part.type = irob_msgs::msg::InstrumentJawPart::SCISSORS;
      } else {
  ...
  virtual void robotActionCB(const std::shared_ptr<rclcpp_action::ServerGoalHandle<irob_msgs::action::RobotAction>> goal_handle) {
    RCLCPP_INFO_STREAM(node->get_logger(), "Start robotActionCB");
    auto goal = goal_handle->get_goal();
    switch (goal->action) {
      case irob_msgs::action::RobotAction::Goal::STOP:
        stop();
        break;
      case irob_msgs::action::RobotAction::Goal::RESET_POSE:
        resetPose(goal->move_allowed);
        break;
      case irob_msgs::action::RobotAction::Goal::FOLLOW_TRAJECTORY:
        followTrajectory(goal->trajectory);
        break;
      case irob_msgs::action::RobotAction::Goal::MOVE_JOINT:
        moveJointAbsolute(goal->joint_state, 0.01);
        break;
      default:
        RCLCPP_ERROR_STREAM(node->get_logger(),
            arm_name << ": invalid robot action code received");
        irob_msgs::action::RobotAction::Result result;
