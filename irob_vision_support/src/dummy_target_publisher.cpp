...
int main(int argc, char **argv)
{
	...
	// Initialize ros node
    ros::init(argc, argv, "dummy_target_publisher");
    ros::NodeHandle nh;
	...
	ros::Publisher vis_pub = 
		nh.advertise<visualization_msgs::Marker>( "dummy_target_marker", 0 );
	...
	try	{
		...
		Trajectory<Eigen::Vector3d> tr = TrajectoryFactory::
				circleTrajectoryHorizontal(
		...
		while (ros::ok())
		{
			...
			marker.pose.position 
					= wrapToMsg
					<geometry_msgs::Point, Eigen::Vector3d>(tr[j % tr.size()]);
