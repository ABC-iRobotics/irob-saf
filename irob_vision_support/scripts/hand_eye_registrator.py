...
class HandEyeRegistrator:
    ...
    def __init__(self):
        ...
        # Subscribe to topics
        rospy.Subscriber("fiducial_tf", PoseStamped, self.cb_fiducial)
        rospy.Subscriber("/" + self.arm + "/measured_cp",
                        PoseStamped, self.cb_measured_cp)
    ...
    def gather_actual_position(self):
        ...
        if ((rospy.Time.now().to_sec()
            ...
            fiducial_pos = np.array([self.fiducial_tf.pose.position.x,
                                 self.fiducial_tf.pose.position.y,
                                 self.fiducial_tf.pose.position.z]).T
            if self.mode == "save":
    ...
    def move_tcp_to(self, target, v, dt):
        ...
        while self.measured_cp is None and not(rospy.is_shutdown()):
        	print("Waitong for measured pose...")
        	rate.sleep()
