...
for i = 1:size(prev_im_coord_L, 2)
   ...
   tempColumnVector = disparityMap(:,uint32(prev_im_coord_L(1,i)));
   sizeDM = size(disparityMap(:,uint32(prev_im_coord_L(1,i))));
   
   for j = uint32(prev_im_coord_L(2, i)): sizeDM(1,1)
   ...
   for j = uint32(prev_im_coord_L(2, i)):-1: 2
       %disp('a')
         diff = abs(tempColumnVector(j) - tempColumnVector(j-1));
         if diff > threshSegmentationDiff
