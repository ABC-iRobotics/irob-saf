...
PegTransferDual::PegTransferDual(ros::NodeHandle nh,
...
void PegTransferDual::doPegTransfer()
{
  ...
  Pose transfer_phantom(Eigen::Vector3d(30.0, 30.0, 70.0), ori_phantom, 0.0);

  Pose dist_pose(dp_phantom.transform(e.tf_phantom));
  Pose transfer_pose(transfer_phantom.transform(e.tf_phantom));
  ...
  Pose transfer_approach_phantom(
        Eigen::Vector3d(0.0, 0.0, 10.0), ori_phantom, 0.0);
  ...
  Eigen::Vector3d transfer_approach(
        transfer_approach_phantom.transform(e.tf_phantom).position
        - Pose().transform(e.tf_phantom).position);
  ...
  while(ros::ok())
  {
    ...
    arms[1]->grasp(grasp_pose_on, approach_pose_on, d, compress_rate, speed_cartesian, speed_jaw);
    while(!arms[1] -> isSurgemeDone() && ros::ok())
    {
      ros::Duration(0.1).sleep();
    }
    ...
    Pose grasp_pose_transfer(transfer_pose);
    grasp_pose_transfer -= grasp_translate;
    Pose approach_pose_transfer(transfer_pose);
    approach_pose_transfer -= grasp_translate;
    std::vector<Pose> waypoints;
    waypoints.push_back(approach_pose_on);
    //waypoints.push_back(dist_pose);
    arms[1] -> place(grasp_pose_transfer, approach_pose_transfer, speed_cartesian, waypoints);
    while(!arms[1] -> isSurgemeDone() && ros::ok())
    {
      ros::Duration(0.1).sleep();
    }

    // Grasp object at transfer
    grasp_pose_transfer += 2.0 * grasp_translate;
    approach_pose_transfer += 2.0 * grasp_translate;
    approach_pose_transfer += transfer_approach;
    d = e.objects[tube_idx_on].grasp_diameter;
    arms[0]->grasp(grasp_pose_transfer, approach_pose_transfer, d, compress_rate, speed_cartesian, speed_jaw);
    ...
    // Release
    approach_pose_transfer -= 2.0 * grasp_translate;
    arms[1] -> release(approach_pose_transfer, d, speed_cartesian, speed_jaw);
    ...
    std::vector<Pose> waypoints_2;
    waypoints_2.push_back(approach_pose_on);
    //waypoints.push_back(dist_pose);
    arms[0] -> place(grasp_pose_to, approach_pose_to, speed_cartesian, waypoints_2);
    while(!arms[0] -> isSurgemeDone() && ros::ok())
    {
      ros::Duration(0.1).sleep();
    }
...
int main(int argc, char **argv)
{
  ...
  try {
    PegTransferDual pnp(nh, arm_names);
