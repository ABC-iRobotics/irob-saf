...
#include <irob_motion/maneuver_controller.hpp>
...
void ManeuverServer::maneuverActionCB(
		const irob_msgs::ManeuverGoalConstPtr &goal)
{
    switch(goal -> action)
    {
    	case irob_msgs::ManeuverGoal::DISSECT:
    	{
    	...
    	case irob_msgs::ManeuverGoal::GRASP:
    	{
    		std::vector<Pose> waypoints;
			for (geometry_msgs::Pose p : goal->waypoints)
				waypoints.push_back(Pose(p, 0.0));
    	...
    	case irob_msgs::ManeuverGoal::MOVE_TO:
    	{
    		std::vector<Pose> waypoints;
			for (geometry_msgs::Pose p : goal->waypoints)
				waypoints.push_back(Pose(p, 0.0));
    	...
    	default:
    	{
    		...
    		irob_msgs::ManeuverResult result;
    			...
    			std::vector<Pose> waypoints)
{
    ...
    irob_msgs::ManeuverFeedback feedback;
    irob_msgs::ManeuverResult result;
