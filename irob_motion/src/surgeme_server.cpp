...
void SurgemeServer::handle_accepted(const std::shared_ptr<GoalHandle> gh)
{
  ...
  std::thread([this, gh]() {
    ...
    switch (goal->action) {
      case irob_msgs::action::Surgeme_Goal::GRASP:       do_grasp(gh); break;
      case irob_msgs::action::Surgeme_Goal::RELEASE:     do_release(gh); break;
      case irob_msgs::action::Surgeme_Goal::NAV_TO_POS:  do_nav_to_pos(gh); break;
      default:
